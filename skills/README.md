<!-- omit from toc -->
# PocketOps Retained Agent Skills

> [!NOTE]
> `skills/` 目录及其说明文档保留自上游 AI Edge Gallery Agent Skills
> 子系统。它们对保留的 Gallery 壳、Agent Skills、JS skill 与
> `run_js` / `run_intent` 路径仍然有参考价值，但并不属于默认
> `PocketOpsActivity + GenieHttpService` 工业诊断主线。

If you are working on the current PocketOps production flow, start with
`com.pocketops.app.PocketOpsActivity` instead. Use this document only when you
intentionally need the retained Agent Skills subsystem.
